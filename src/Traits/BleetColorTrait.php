...
trait BleetColorTrait
{
    ...
    {
        if (method_exists($this, 'hasErrors') === true && $this->hasErrors() === true) {
    ...
    protected function textStrongColorClass(): string
    {
        return match ($this->getEffectiveColor()) {
    ...
    protected function inputColorClasses(): array
    {
        return [
            ...
            $this->outlineColorClass(),
            $this->placeholderColorClass(),
            $this->focusOutlineColorClass(),
        ];
    }

    /**
     * Field outline at rest. Tailwind Plus styles the fields with outline, not
     * with border: the border does not shift the layout when it thickens on
     * focus.
     */
    protected function outlineColorClass(): string
    {
        return match ($this->getEffectiveColor()) {
    ...
    protected function focusOutlineColorClass(): string
    {
        return match ($this->getEffectiveColor()) {
            Bleet::COLOR_PRIMARY => 'focus:outline-primary-600',
            Bleet::COLOR_SECONDARY => 'focus:outline-secondary-600',
            Bleet::COLOR_SUCCESS => 'focus:outline-success-600',
            Bleet::COLOR_DANGER => 'focus:outline-danger-600',
            Bleet::COLOR_WARNING => 'focus:outline-warning-600',
            Bleet::COLOR_INFO => 'focus:outline-info-600',
            Bleet::COLOR_ACCENT => 'focus:outline-accent-600',
        };
    }
    ...
    {
        return match ($this->getEffectiveColor()) {
            Bleet::COLOR_PRIMARY => 'placeholder:text-primary-400',
            Bleet::COLOR_SECONDARY => 'placeholder:text-secondary-400',
            Bleet::COLOR_SUCCESS => 'placeholder:text-success-400',
            Bleet::COLOR_DANGER => 'placeholder:text-danger-400',
            Bleet::COLOR_WARNING => 'placeholder:text-warning-400',
            Bleet::COLOR_INFO => 'placeholder:text-info-400',
            Bleet::COLOR_ACCENT => 'placeholder:text-accent-400',
        };
    }
