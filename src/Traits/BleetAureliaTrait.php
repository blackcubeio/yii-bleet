...
/**
 * Trait for the high level Aurelia helpers (overlay, tabs, pager, burger, menu)
 */
trait BleetAureliaTrait
{
    ...
    public static function overlay(array $options = []): string
    {
        ...
        if (is_string($existingClasses) === true) {
    ...
    {
        return ['bleet-tabs' => is_string($options) === true ? $options : Aurelia::attributesCustomAttribute($options)];
    }
    ...
    {
        return ['bleet-pager' => is_string($options) === true ? $options : Aurelia::attributesCustomAttribute($options)];
    }
    ...
    {
        return ['bleet-burger' => is_string($options) === true ? $options : Aurelia::attributesCustomAttribute($options)];
    }
    ...
    {
        return ['bleet-menu' => is_string($options) === true ? $options : Aurelia::attributesCustomAttribute($options)];
    }
}
