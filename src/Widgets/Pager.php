 ...
 * @link https://www.blackcube.io
 */
class Pager extends AbstractWidget
{
    use RenderViewTrait;
    ...
    /**
     * Hides the widget when a single page exists
     */
    ...
    {
        if ($this->paginator->isPaginationRequired() === false && $this->hideOnSinglePage) {
            return '';
        }
    ...
    private function prepareViewParams(): array
    {
        ...
        $prevPage = $this->paginator->isOnFirstPage() === true ? null : [
            'number' => $currentPage - 1,
            'url' => $this->generatePageUrl($currentPage - 1),
        ];

        $nextPage = $this->paginator->isOnLastPage() === true ? null : [
            'number' => $currentPage + 1,
            'url' => $this->generatePageUrl($currentPage + 1),
        ];
