...
class Sidebar extends AbstractWidget
{
     ...
     * Use encode(false) to pass raw HTML (inline SVG, and so on)
     */
    ...
    private function prepareViewParams(): array
    {
        ...
        return [
            ...
            'headerMobileClasses' => ['flex', 'items-center', 'justify-between', 'p-4', 'lg:hidden', ...($this->showSeparator === true ? ['border-b', ...$this->getBorderColorClasses()] : [])],
            'closeButtonClasses' => ['-m-2.5', 'p-2.5', 'cursor-pointer', ...$this->getCloseButtonColorClasses()],
            'headerDesktopClasses' => ['hidden', 'lg:flex', 'items-center', 'p-4', ...($this->showSeparator === true ? ['border-b', ...$this->getBorderColorClasses()] : [])],
