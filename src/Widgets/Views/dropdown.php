 ...
 * @var bool $withTags
 * @var bool $disabled
 * @var array<int, string|int> $disabledOptions
 * @var array $itemDisabledClasses
 ...
 * @var array $checkIconClasses
 * @var array $disabledClasses
...
<?php if ($prompt !== '' && $multiple === false): ?>
    <?php echo Html::tag('option', Html::encode($prompt), ['value' => '']); ?>
<?php endif; ?>
<?php $selectedStrings = array_map('strval', $selected); ?>
<?php $disabledStrings = array_map('strval', $disabledOptions ?? []); ?>
<?php foreach ($options as $key => $optionValue): ?>
    <?php if (is_array($optionValue) === true): ?>
        <?php echo Html::openTag('optgroup', ['label' => $key]); ?>
        <?php foreach ($optionValue as $value => $optionLabel): ?>
            <?php
            $optionAttributes = ['value' => $value];
            if (in_array((string)$value, $selectedStrings, true) === true) {
                $optionAttributes['selected'] = true;
            }
            if (in_array((string)$value, $disabledStrings, true) === true) {
    ...
    <?php else: ?>
        ...
        if (in_array((string)$key, $selectedStrings, true) === true) {
            $optionAttributes['selected'] = true;
        }
        if (in_array((string)$key, $disabledStrings, true) === true) {
            $optionAttributes['disabled'] = true;
        }
        ?>
...
if ($disabled) {
    ...
    $buttonAttributes['class'] = array_merge((array)$buttonAttributes['class'], $disabledClasses);
}
?>
<?php echo Html::openTag('button', $buttonAttributes); ?>
<?php if ($withTags === true): ?>
    <span data-dropdown="tags" class="col-start-1 row-start-1 flex flex-wrap gap-1 pr-6">
        <span data-dropdown="placeholder" class="text-secondary-400"><?php echo Html::encode($prompt); ?></span>
    </span>
<?php else: ?>
    <span data-dropdown="value" class="col-start-1 row-start-1 truncate pr-6"></span>
<?php endif; ?>
    <?php echo Svg::heroicon()->solid('chevron-down')->addClass(...$chevronClasses); ?>
...
<template data-dropdown="tag-template">
    <?php echo Html::openTag('span', ['class' => $tagClasses, 'data-tag-value' => true]); ?>
        ...
        <?php echo Html::openTag('button', [
            ...
            <?php echo Svg::heroicon()->solid('x-mark')->addClass(...$tagRemoveSvgClasses); ?>
        <?php echo Html::closeTag('button'); ?>
...
<?php if ($searchable === true): ?>
    <div class="sticky top-0 z-10 grid grid-cols-1 border-b border-secondary-100 bg-white">
        <?php echo Html::input('text', '', '', [
            'data-dropdown' => 'search',
            'class' => $searchClasses,
            'placeholder' => $searchPlaceholder,
        ]); ?>
        <?php echo Svg::heroicon()->solid('magnifying-glass')->addClass(...$searchIconClasses); ?>
    </div>
<?php endif; ?>
    ...
    <template data-dropdown="item-template">
        <?php echo Html::openTag('button', [
            ...
            'data-class-disabled' => implode(' ', $itemDisabledClasses ?? []),
        ]); ?>
            ...
            <?php echo Html::openTag('span', [
                ...
                <?php echo Svg::heroicon()->solid('check')->addClass(...$checkIconClasses); ?>
            <?php echo Html::closeTag('span'); ?>
        <?php echo Html::closeTag('button'); ?>
    </template>
