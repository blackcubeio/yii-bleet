...
class Anchor extends AbstractWidget
{
    ...
    private function buildContent(): ?string
    {
        ...
        if ($this->iconName !== null) {
            $iconSize = $this->outline === true ? 'size-4' : 'size-6';
    ...
    protected function prepareClasses(): array
    {
        ...
        $flexClasses = ($hasIconAndContent === true || $this->button === true) ? ['inline-flex', 'items-center', 'gap-2'] : [];
        if ($this->button === true) {
            $flexClasses[] = 'justify-center';
        }

        if ($this->outline === true && $this->button === true) {
            $baseClasses = [
                'rounded-md',
                'font-semibold',
                'cursor-pointer',
    ...
    {
        if ($this->outline === true && $this->button === false) {
        ...
        if ($this->ghost === true || $this->button === true) {
            return match ($this->size) {
