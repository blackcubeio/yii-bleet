...
| `Bleet::select()` | Select | Select with custom display (`searchable()`, `multiple()`, `withTags()`) |
...
| `Bleet::buttonGroup()` | ButtonGroup | Grouped action buttons |
...
| `Bleet::svg()` / `Bleet::icon()` | `Blackcube\Icons\Svg` | Heroicons / custom SVG |
...
| `AjaxifyAction` | `Refresh`, `Run` | Secondary ajaxify action |
...
Styling shared by every Bleet field: the color setters and the classes of the
three parts of the block — label, hint, error.

Model binding is no longer done here: fields extend
[blackcube/yii-form](https://github.com/blackcubeio/yii-form), which reads the
model through `yiisoft/form`. Pass the model to the factory:

```php
Bleet::input($model, 'email')->render();
```
