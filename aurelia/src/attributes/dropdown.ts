...
export class BleetDropdownCustomAttribute {
            ...
            if (isClosed) {
                ...
                return this.trapFocusService.start(
                    ...
                    .then(() => {
                        ...
                        // The panel opens on its SELECTION, not on its first item. With a search field the focus goes
                        // to the field, so nothing brings the selected item into view any more: on a list of thirty
                        // entries the current value stayed below the fold (measured on a project list, 2026-09-06).
                        // `nearest` scrolls the panel only when the item is out of sight, and never jumps the page.
                        // After `remove('hidden')`: a hidden element has no geometry to scroll to.
                        (this.itemsPlace?.querySelector('[aria-selected="true"]') as HTMLElement | null)
    ...
    private onClickToggleItem = (event: MouseEvent) => {
        ...
        if (!element) return;
        // A disabled option is shown but never chosen: the native `<option disabled>` is the source of truth.
    ...
    private updateButtonText() {
        ...
        if (selectedOptions.length === 0) {
            ...
            (this.buttonText as HTMLElement).textContent = `${selectedOptions.length} selected`;
        }
    }
    ...
    private appendOption(option: HTMLOptionElement) {
        ...
        // `<option disabled>` → the item stays visible (so the reader knows it exists) but cannot be taken: no click,
        // no focus, and the classes the template declares for that state (`data-class-disabled`).
        if (option.disabled) {
            button.disabled = true;
            button.setAttribute('aria-disabled', 'true');
            const disabledClasses = button.dataset.classDisabled?.split(' ').filter((c) => c !== '') || [];
