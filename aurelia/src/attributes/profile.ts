...
export class BleetProfileCustomAttribute
{
    ...
    public attached()
    {
        ...
        this.disposable = this.ea.subscribe(Channels.Profile, this.onProfileEvent);
    ...
    private onProfileEvent = (data: IProfile) => {
        this.logger.trace('onProfileEvent', data);
        const isTargeted = (data.id === undefined || data.id === this.id);
        if (data.action === ProfileAction.Close && isTargeted && this.isOpen) {
            this.logger.trace('Profile Close action received');
            this.close();
        }
    }
