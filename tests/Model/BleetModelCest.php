...
class BleetModelCest
{
    ...
    public function testModelLabelsHintsPlaceholders(ModelTester $I): void
    {
        ...
        $I->assertSame('Full name', $model->getPropertyLabel('name'));
        $I->assertSame('Email address', $model->getPropertyLabel('email'));

        $I->assertSame('Your first and last name', $model->getPropertyHint('name'));
        $I->assertSame('A valid email is required', $model->getPropertyHint('email'));
